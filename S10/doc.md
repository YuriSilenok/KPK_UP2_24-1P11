...
| `status` | Status для точного совпадения | string |
...
#### 6.5 Получить список должностей (`list_positions`)
...
#### 7.4 Получить назначение по ID (`get_employee_position`)
...
- **Выходные данные:** Измененная запись отпуска с лимитом `max_length=255`.
