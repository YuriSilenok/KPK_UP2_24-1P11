...
| `is_active` | Фильтр по статусу удаления (Замечание 2) | boolean |
...
| `status` | Текущий статус | string |
| `is_active` | Статус активности записи | boolean |
| `position_id` | Идентификатор должности. Получается путем INNER JOIN выборки активной записи из транзитивной таблицы `employee_positions` по условию равенства `employee_id`. (Замечание 3) | int |
...
| Параметр | Пояснение | Обязательность | Тип | Ограничение | Значение по умолчанию |
|----------|-----------|----------------|-----|-------------|-----------------------|
| `employee_id` | ID связанного сотрудника | Да | int | должен существовать | – |
| `position_id` | ID связанной должности | Да | int | должен существовать | – |
...
- **Изменить**: Принимает `id` (int) и изменяемые параметры периода или ставки. Возвращает измененный объект связи.
- **Удалить**: Принимает `id` (int). Возвращает `True / False` (bool).
- **Получить список**: Принимает параметры фильтрации `employee_id` или `position_id`. Возвращает список объектов связи.
...
| `end_date` | Дата окончания отпуска | Да | date | не раньше start_date | – |
...
- **Изменить**: Принимает `id` (int) и обновляемые поля дат или типа отпуска. Возвращает измененный объект.
- **Удалить**: Принимает `id` (int). Осуществляет мягкое удаление (флаг `is_active = False`). Возвращает `True / False` (bool).
- **Получить список**: Фильтрует по `employee_id` и `is_active=True`. Возвращает список записей отпусков.
...
| `end_date` | Дата окончания больничного | Да | date | не раньше start_date | – |
...
- **Изменить**: Принимает `id` (int) и обновляемые поля дат или диагноза. Возвращает измененный объект.
- **Удалить**: Принимает `id` (int). Осуществляет мягкое удаление (флаг `is_active = False`). Возвращает `True / False` (bool).
- **Получить список**: Фильтрует по `employee_id` и `is_active=True`. Возвращает список записей больничных.
...
### Список реляционных связей (Замечание 1)
