...
- `groups.id` → `curriculums.group_id (CASCADE)`
- `disciplines.id` → `curriculums.discipline_id (CASCADE)`
- `semesters.id` → `curriculums.semester_id (CASCADE)`
...
erDiagram
    ...
    groups ||--o{ curriculums : "groups.id → curriculums.group_id (CASCADE)"
    disciplines ||--o{ curriculums : "disciplines.id → curriculums.discipline_id (CASCADE)"
    semesters ||--o{ curriculums : "semesters.id → curriculums.semester_id (CASCADE)"
```
...
| :--- | :--- | :--- |
| success | bool | true (если запись найдена и помечена удаленной), false (если запись не найдена или уже удалена ранее) |
...
| assessment_form | Форма контроля | Да | string | exam / credit (ввод иных значений вызывает ошибку валидации 400) | — |
| is_active | Статус активности записи | Нет | bool | — | true |
...
| assessment_form | Новая форма контроля | Нет | string | exam / credit (ввод иных значений вызывает ошибку валидации 400) |
| is_active | Восстановление/деактивация записи | Нет | bool | — |
...
2. **Получение плана конкретной группы с диапазоном часов**: Передается параметр `group_id` совместно с фильтрами `theory_hours_min` и `theory_hours_max` для гибкого подбора записей под учебные нормативы.
3. **Фильтрация по форме контроля**: Параметр `assessment_form` комбинируется с `group_id` или `semester_id` для выявления всех экзаменов/зачетов группы в семестре.
