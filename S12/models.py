...
class Subject(BaseModel):
    id = AutoField()
    curriculum_plan_id = ForeignKeyField(CurriculumPlan, backref='subjects', on_delete='CASCADE', null=False)
    name = CharField(null=False, constraints=[Check("name != ''")])
...
class Hours(BaseModel):
    id = AutoField()
    subject_id = ForeignKeyField(Subject, backref='hours', on_delete='CASCADE', null=False, unique=True)
...
class Assessment(BaseModel):
    id = AutoField()
    subject_id = ForeignKeyField(Subject, backref='assessments', on_delete='CASCADE', null=False)
    type = CharField(null=False, constraints=[Check("type IN ('exam', 'zachet')")])
    is_active = BooleanField(default=True)

    class Meta:
        indexes = (
            (('subject_id', 'type'), True),
        )
