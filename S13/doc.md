...
* Привязка программ к специальностям и конкретным дисциплинам через транзитивную таблицу.
* Учет версий программ и путей к файлам.
* Правила валидации: поля не могут быть пустыми (Not NULL), уникальность связки "название + версия".
...
Информация, возвращаемая в случае удачного изменения (полный объект):
| Параметр | Тип | Описание |
| :--- | :--- | :--- |
...
Информация, требуемая для получения списка (фильтрация):

| Параметр | Тип | Описание |
| :--- | :--- | :--- |
| title | string | Поиск по частичному совпадению в названии (регистронезависимый) |
| version | string | Поиск по конкретному номеру версии |
| discipline_id | int | Поиск программ, привязанных к конкретному ID дисциплины |
...
### ER-диаграмма
Диаграмма разработана в соответствии с требованиями 3НФ. Связь "Многие-ко-многим" (Many-to-Many) между сущностями **WorkProgram** и **Specialty** реализована через транзитивную таблицу **ProgramAssignment**. Для связи с учебным процессом в таблицу назначений добавлено поле `discipline_id`.
