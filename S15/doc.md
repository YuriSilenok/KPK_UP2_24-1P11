...
### Описание
Удаление логическое — поле `is_deleted` устанавливается в `true`. При успехе возвращается HTTP 204 без тела.
...
### Параметры запроса
| Параметр | Тип | Обязательность | Ограничение | Значение по умолчанию | Пояснение |
|----------|-----|----------------|-------------|-----------------------|-----------|
...
| limit | integer | Нет | >0 | 100 | Лимит записей |
| offset | integer | Нет | >=0 | 0 | Сдвиг |
...
- `Student.current_group_id` → `Group.id` (N:1)
