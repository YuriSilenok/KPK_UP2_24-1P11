...
- True — аудитория успешно удалена.
- False — аудитория не найдена.
...
Если параметры фильтрации не указаны, возвращаются все аудитории, не помеченные как удаленные.
...
Поле `campus_id` хранит только идентификатор корпуса.

Данные о корпусах находятся в Campus Service (Вариант 16).

Интеграция между сервисами в рамках данного задания не реализуется.
