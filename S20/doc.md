...
**Тип удаления:** логическое (soft delete). Запись не удаляется из базы данных физически, а устанавливается значение `is_active = False`. Запись остается в БД, но исключается из стандартных выборок. Сервис не входит в список исключений для физического удаления.

### Выходные данные

Возвращается простое логическое значение (без упаковки в объект/поле):

| Тип | Пояснение |
|-----|-----------|
| Логический | `true` — если учебный период успешно деактивирован (установлен `is_active = False`), `false` — если период с указанным ID не найден или уже деактивирован |
...
| Параметр | Пояснение | Тип | Обязательность | Ограничение |
|----------|-----------|-----|----------------|-------------|
| academic_year | Фильтр по учебному году | Строка | Необязательно | Формат ГГГГ-ГГГГ |
| period_type | Тип периода | Строка | Необязательно | semester, module |
| name_contains | Поиск по части имени | Строка | Необязательно | 1-100 символов |
| parent_period_id | Фильтр по родительскому периоду | Целое | Необязательно | 0 – корневые, ID существующего семестра |
