...
**Тип удаления:** мягкое (логическое). Запись не удаляется из базы данных физически, а устанавливается значение `is_active = False`. Запись остается в БД, но исключается из выборок по умолчанию.

### Выходные данные

| Параметр | Тип | Пояснение |
|----------|-----|-----------|
| result | Логический | True, если учебный период успешно деактивирован (установлен is_active = False), иначе False |
