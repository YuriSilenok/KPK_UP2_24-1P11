...
- `create_schedule(group_id, subgroup_id, day_of_week)` – создание расписания.
- `update_schedule(id, group_id, subgroup_id, day_of_week)` – изменение расписания.
- `delete_schedule(id)` – удаление расписания.
- `get_schedule(id)` – получение расписания по ID.
- `list_schedules(group_id, subgroup_id, day_of_week)` – получение списка расписаний с фильтрацией.
...
- `list_timeslots(schedule_id, group_id, subgroup_id, day_of_week, order_number, is_lesson)` – получение списка слотов с фильтрацией.

#### Правила валидации данных

- Для временного слота: `start_time` < `end_time`; `order_number` > 0; комбинация (`schedule_id`, `order_number`) уникальна.
- Для расписания: комбинация (`group_id`, `subgroup_id`, `day_of_week`) уникальна.
- `group_id` проверяется заглушкой (допустимые значения 1..20).
- `subgroup_id`: `0` означает основную группу, от 1 до 50 – реальные подгруппы.
- `day_of_week` должен быть в диапазоне 1..7.
...
| subgroup_id      | Обязательно    | Целое  | 0 – группа целиком, 1..50 – подгруппа      | —                     |
| day_of_week      | Обязательно    | Целое  | 1..7 (день недели)                         | —                     |

Уникальные комбинации: (group_id, subgroup_id, day_of_week)
...
| group_id         | Целое       | Фильтр по группе (опционально)                      |
| subgroup_id      | Целое       | Фильтр по подгруппе (0 – группа, 1..50)             |
| day_of_week      | Целое       | Фильтр по дню недели (опционально)                  |
