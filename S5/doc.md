# Вариант №5
# Сервис управления отделениями/факультетами СПО (Department Service)

## Сущность: Department (Отделение/Факультет)

### 1. Информация для создания сущности

| Параметр | Обязательность | Тип | Ограничение | Значение по умолчанию |
|----------|----------------|-----|-------------|-----------------------|
| `name` | Да | str | длина от 3 до 50 символов, уникально | — |
| `phone` | Нет | str | формат `+7XXXXXXXXXX` (12 символов) или пустая строка | `''` (пустая строка) |

**Уникальные комбинации параметров:**  
- `name` (уникально само по себе)

### 2. Информация, возвращаемая при успешном создании

| Параметр | Тип |
|----------|-----|
| `id` | int |
| `name` | str |
| `phone` | str |

## Изменить сущность по ID

### 3. Информация для изменения сущности

| Параметр | Обязательность | Тип | Ограничение | Значение по умолчанию |
|----------|----------------|-----|-------------|-----------------------|
| `name` | Нет | str | длина от 3 до 50 символов, уникально (кроме самого себя) | текущее значение |
| `phone` | Нет | str | формат `+7XXXXXXXXXX` (12 символов) или пустая строка | текущее значение |

### 4. Информация, возвращаемая при успешном изменении

| Параметр | Тип |
|----------|-----|
| `id` | int |
| `name` | str |
| `phone` | str |


## Удалить сущность по ID

Вернет `True` (удалено) или `False` (не найдено) в поле `deleted`.


## Получить сущность по ID

### 5. Информация, возвращаемая при успешном поиске

| Параметр | Тип |
|----------|-----|
| `id` | int |
| `name` | str |
| `phone` | str |


## Получить список сущностей по заданным параметрам

### 6. Параметры для получения списка

| Параметр | Тип | Описание |
|----------|-----|-----------|
| `name` | str | Поиск по части названия отделения |
| `limit` | int | Максимум записей (по умолчанию 50) |

### 7. Возвращаемый список сущностей

| Параметр | Тип |
|----------|-----|
| `id` | int |
| `name` | str |
| `phone` | str |

![ER-диаграмма](./erd.png)

PK — Primary Key (первичный ключ)

UK — Unique Key (уникальный ключ)