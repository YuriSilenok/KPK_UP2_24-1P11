...
Входные параметры для обновления данных группы. Поля `name` и `formation_year` не подлежат изменению в данной операции, поэтому они полностью исключены как из входных, так и из выходных данных. Поле `id` также является неизменяемым и служит исключительно для идентификации сущности в базе данных.
